...
function Header() {
  ...
  const drawer = (
    <Box onClick={handleDrawerToggle} sx={{ textAlign: 'center' }}>
      <Typography variant="h6" sx={{ my: 2, color: '#171412', fontWeight: 'bold' }}>
        Classic Rides
      </Typography>
      <List>
        {navItems.map((item) => (
          <ListItem key={item.name} disablePadding>
            <ListItemButton component={Link} href={item.href} sx={{ textAlign: 'center', color: '#171412' }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, color: '#171412' }}>
          ...
          <Typography variant="h6" sx={{ color: '#171412', fontWeight: 'bold', letterSpacing: '-0.015em' }}>
            Classic Rides
          </Typography>
        </Box>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 2, md: 4 } }}>
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  sx={{ color: '#171412', fontSize: '0.875rem', fontWeight: 'medium', textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}
                >
                  {item.name}
                </Link>
              ))}
